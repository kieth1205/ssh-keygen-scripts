...
#
# 🐧 Linux SSH Setup
# 🔐 SSH configuration script for Linux systems
...
echo -e "${GREEN}Setting up MediKoGPT SSH Setup Tool for Linux...${NC}"
...
if [ -n "$ZSH_VERSION" ]; then
    ...
    echo -e "${YELLOW}Could not detect shell type. Using ~/.bashrc${NC}"
    CONFIG_FILE=~/.bashrc
fi
